...
var bisectRight = ascendingBisect.right;var e10 = Math.sqrt(50),
    e5 = Math.sqrt(10),
    e2 = Math.sqrt(2);
...
selection.prototype.transition = selection_transition;var pi = Math.PI,
    tau = 2 * pi,
    epsilon$1 = 1e-6,
...
var csvParse = csv.parse;function responseText(response) {
...
var csv$1 = dsvParse(csvParse);// Computes the decimal coefficient and exponent of the specified number x with
...
var slice$1 = array.slice;var implicit = {name: "implicit"};
...
var durationWeek = 6048e5;var day$1 = newInterval(function(date) {
...
var saturday = weekday(6);var year = newInterval(function(date) {
  date.setMonth(0, 1);
  date.setHours(0, 0, 0, 0);
}, function(date, step) {
...
var utcSaturday = utcWeekday(6);var utcYear = newInterval(function(date) {
...
function formatDayOfYear(d, p) {
  return pad$1(1 + day$1.count(year(d), d), p, 3);
}
...
const main = async () => {
  ...
  // Set up data
  let confirmedData = await csv$1(confirmedUrl);
  let deathsData = await csv$1(deathsUrl);
  let recoveredData = await csv$1(recoveredUrl);

  let actionData = await csv$1("./lockdown-dates-03-14-2020.csv");
  ...
  function update(data, yLinear) {
    ...
    subGroups
      ...
      .attr("d", d => line$1(d.series.map(v => v[yValue])));
